...
router.put('/:id', auth, async (req, res) => {
  ...
  try {
    ...
    const [updatedLeads] = await db.execute(
      `SELECT * FROM leads WHERE id = ?`,
      [req.params.id]
    );
...
router.get('/stats/overview', auth, async (req, res) => {
  try {
    ...
    const [totalLeadsRow] = await db.execute(
      `SELECT COUNT(*) as totalLeads FROM leads ${where}`,
      params
    );
    const [totalAmountRow] = await db.execute(
...
router.get('/stats/win-loss', auth, async (req, res) => {
  try {
    ...
    const [stageAnalysis] = await db.execute(`
      SELECT 
        stage,
    ...
    const [stageTransitions] = await db.execute(`
      SELECT 
        stage,
